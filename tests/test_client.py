...
class TestCreateSynapseResources:
                ...
                "children": [
                    {
                        "name": "Genes",
                        "type": "Folder",
                        "children": [{"name": "testing", "type": "Folder"}],
                    }
                ],
            }
        ]
    ...
    def test__create_synapse_resources_project(self):
        ...
        project_config = [{"name": project_name, "type": "Project"}]
        expected_config = [{"name": project_name, "type": "Project", "id": "syn12222"}]
        project_ent = synapseclient.Project(id="syn12222")
        with patch.object(
            self.create_cls, "get_or_create_project", return_value=project_ent
        ) as patch_create:
            client._create_synapse_resources(
                config_list=project_config, creation_cls=self.create_cls
            )
    ...
    def test__create_synapse_resources_folder(self):
        ...
        folder_config = [
            {"name": "Test 1", "type": "Folder"},
            {"name": "Test 2", "type": "Folder"},
        ]
        expected_config = [
            {"name": "Test 1", "type": "Folder", "id": "syn33333"},
            {"name": "Test 2", "type": "Folder", "id": "syn22222"},
        ]
        ...
        with patch.object(
            ...
            side_effect=[folder_ent_1, folder_ent_2],
        ) as patch_create:
            client._create_synapse_resources(
                config_list=folder_config,
    ...
    def test__create_synapse_resources_acl(self):
        ...
        project_name = "Test Configuration"
        project_config = [{"name": project_name, "type": "Project", "acl": ["fake"]}]
        expected_config = [
            {"name": project_name, "type": "Project", "id": "syn12222", "acl": ["fake"]}
        ]
        project_ent = synapseclient.Project(id="syn12222")
        with patch.object(
            self.create_cls, "get_or_create_project", return_value=project_ent
        ) as patch_create, patch.object(create, "_set_acl") as patch_set:
            client._create_synapse_resources(
                config_list=project_config, creation_cls=self.create_cls
            )
            patch_create.assert_called_once_with(name=project_name)
            ...
            patch_set.assert_called_once_with(
                syn=self.create_cls.syn, entity=project_ent, acl_config=["fake"]
            )
    ...
    def test__create_synapse_resources_recursive(self):
        ...
        with patch.object(
            ...
            self.create_cls, "get_or_create_folder", return_value=folder_ent
        ) as patch_create_folder:
            client._create_synapse_resources(
                config_list=self.config, creation_cls=self.create_cls
            )
            patch_create_proj.assert_called_once_with(name="Test Configuration")
    ...
    def test__create_synapse_resources_team(self):
        ...
        with patch.object(
            self.create_cls, "get_or_create_team", return_value=team_ent
        ) as patch_create:
            client._create_synapse_resources(
                config_list=team_config, creation_cls=self.create_cls
            )
    ...
    def test__create_synapse_resources_team_invite(self):
        ...
        with patch.object(
            ...
            self.create_cls.syn, "invite_to_team"
        ) as patch_invite:
            client._create_synapse_resources(
                config_list=team_config, creation_cls=self.create_cls
            )
