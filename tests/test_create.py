...
def test__find_by_obj_or_create__onlycreate_raise():
    ...
    with patch.object(
        SYN, "store", side_effect=mocked_409
    ) as patch_syn_store, pytest.raises(
        ValueError, match="foo. To use existing entities, " "set only_get to True."
    ):
...
def test__find_by_obj_or_create__wrongcode_raise():
    ...
    with patch.object(
        SYN, "store", side_effect=mocked_404
    ) as patch_syn_store, pytest.raises(SynapseHTTPError, match="Not Found"):
        CREATE_CLS._find_by_obj_or_create(entity)
        patch_syn_store.assert_called_once_with(entity, createOrUpdate=False)
...
def test__find_by_obj_or_create__get():
    ...
    entity = synapseclient.Entity(
        name=str(uuid.uuid1()), parentId=str(uuid.uuid1()), concreteType=concretetype
    )
    returned = synapseclient.Entity(
        name=str(uuid.uuid1()),
        id=str(uuid.uuid1()),
    ...
    with patch.object(
        SYN, "store", side_effect=mocked_409
    ) as patch_syn_store, patch.object(
        GET_CLS, "_get_obj", return_value=returned
    ) as patch_cls_get:
...
def test__find_entity_by_name__valid():
    ...
    obj = synapseclient.File(path="foo.txt", parentId="syn12345", id="syn11111")
    with patch.object(
        SYN, "findEntityId", return_value=post_return
    ) as patch_find, patch.object(SYN, "get", return_value=obj) as patch_get:
...
def test__find_entity_by_name__invalid():
    ...
    post_return = {"id": "syn11111"}
    obj = synapseclient.File(path="foo.txt", parentId="syn12345", id="syn11111")
    with patch.object(SYN, "findEntityId", return_value=post_return), patch.object(
        SYN, "get", return_value=obj
    ), pytest.raises(AssertionError, match="Retrieved .* had type .* rather than .*"):
        GET_CLS._find_entity_by_name(
            parentid="syn12345", entity_name="foo.txt", concrete_type="Test"
        )
...
@pytest.mark.parametrize(
    "obj",
    [
        synapseclient.Project(name="foo"),
...
def test__get_obj__entity(obj):
    ...
    with patch.object(GET_CLS, "_find_entity_by_name", return_value=obj) as patch_get:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            synapseclient.Evaluation(name="foo", contentSource="syn123"),
            "getEvaluationByName",
        ),
    ],
)
...
def test_get_or_create_project__call():
    ...
    returned = synapseclient.Project(name=project_name, id=str(uuid.uuid1()))
    with patch.object(
        CREATE_CLS, "_find_by_obj_or_create", return_value=returned
    ) as patch_find_or_create:
...
def test_get_or_create_team__call():
    ...
    public_join = True
    team_ent = synapseclient.Team(
        name=team_name, description=description, canPublicJoin=public_join
    )
    returned = synapseclient.Team(
        name=team_name,
    ...
    with patch.object(
        ...
        new_team = CREATE_CLS.get_or_create_team(
            name=team_name, description=description, canPublicJoin=public_join
        )
...
def test_get_or_create_folder__call():
    ...
    parentid = str(uuid.uuid1())
    folder = synapseclient.Folder(name=folder_name, parentId=parentid)
    returned = synapseclient.Folder(
        name=folder_name, id=str(uuid.uuid1()), parentId=parentid
    )
    with patch.object(
        CREATE_CLS, "_find_by_obj_or_create", return_value=returned
    ) as patch_find_or_create:
        new_folder = CREATE_CLS.get_or_create_folder(
...
def test_get_or_create_file__call():
    ...
    parentid = str(uuid.uuid1())
    file_ent = synapseclient.File(path=file_path, parentId=parentid)
    returned = synapseclient.File(
        path=file_path, id=str(uuid.uuid1()), parentId=parentid
    )
    with patch.object(
        CREATE_CLS, "_find_by_obj_or_create", return_value=returned
    ) as patch_find_or_create:
        new_file = CREATE_CLS.get_or_create_file(path=file_path, parentId=parentid)
...
def test_get_or_create_view__call():
    ...
    parentid = str(uuid.uuid1())
    view_ent = synapseclient.EntityViewSchema(name=view_name, parentId=parentid)
    returned = synapseclient.EntityViewSchema(
        name=view_name, id=str(uuid.uuid1()), parentId=parentid
    )
    with patch.object(
        CREATE_CLS, "_find_by_obj_or_create", return_value=returned
    ) as patch_find_or_create:
        new_view = CREATE_CLS.get_or_create_view(name=view_name, parentId=parentid)
...
def test_get_or_create_schema__call():
    ...
    parentid = str(uuid.uuid1())
    schema_ent = synapseclient.Schema(name=schema_name, parentId=parentid)
    returned = synapseclient.Schema(
        name=schema_name, id=str(uuid.uuid1()), parentId=parentid
    )
    with patch.object(
        CREATE_CLS, "_find_by_obj_or_create", return_value=returned
    ) as patch_find_or_create:
        new_schema = CREATE_CLS.get_or_create_schema(
            name=schema_name, parentId=parentid
        )
...
def test_get_or_create_wiki__call():
    ...
    wiki_ent = synapseclient.Wiki(title=wiki_title, markdown=markdown, owner=owner)
    returned = synapseclient.Wiki(
        title=wiki_title, markdown=markdown, id=str(uuid.uuid1()), owner=owner
    )
    with patch.object(
        CREATE_CLS, "_find_by_obj_or_create", return_value=returned
    ) as patch_find_or_create:
        new_wiki = CREATE_CLS.get_or_create_wiki(
            owner=owner, title=wiki_title, markdown=markdown
        )
...
def test_get_or_create_queue__call():
    ...
    queue = synapseclient.Evaluation(
        name=queue_name, contentSource=parentid, description=description, quota={}
    )
    returned = synapseclient.Evaluation(
        name=queue_name,
    ...
    with patch.object(
        ...
        new_queue = CREATE_CLS.get_or_create_queue(
            name=queue_name, contentSource=parentid, description=description, quota={}
        )
...
def test__get_challenge__call():
    ...
    with patch.object(SYN, "restGET", return_value=rest_return) as patch_rest_get:
...
def test__create_challenge__call():
    ...
    input_dict = {"participantTeamId": teamid, "projectId": projectid}
    with patch.object(SYN, "restPOST", return_value=rest_return) as patch_rest_post:
        chal = CREATE_CLS._create_challenge(
            participantTeamId=teamid, projectId=projectid
        )
        patch_rest_post.assert_called_once_with("/challenge", json.dumps(input_dict))
        assert chal == rest_return
...
def test_get_or_create_challenge__create():
    ...
    with patch.object(
        CREATE_CLS, "_create_challenge", return_value=returned
    ) as patch_create:
        new_chal = CREATE_CLS.get_or_create_challenge(
            participantTeamId=teamid, projectId=projectid
        )
        assert new_chal == returned
        patch_create.assert_called_once_with(
            projectId=projectid, participantTeamId=teamid
        )
...
def test_get_or_create_challenge__get():
    ...
    with patch.object(
        GET_CLS, "_create_challenge", side_effect=mocked_400
    ), patch.object(GET_CLS, "_get_challenge", return_value=returned) as patch_get:
        new_chal = GET_CLS.get_or_create_challenge(
            participantTeamId=teamid, projectId=projectid
        )
...
def test_get_or_create_challenge__get_raise():
    ...
    mocked_400 = SynapseHTTPError("foo", response=Mock(status_code=400))
    with patch.object(
        CREATE_CLS, "_create_challenge", side_effect=mocked_400
    ), pytest.raises(
        ValueError, match="foo. To use existing entities, " "set only_get to True."
    ):
        CREATE_CLS.get_or_create_challenge(
            participantTeamId=teamid, projectId=projectid
        )
...
def test_get_or_create_challenge__get_raise_404():
    ...
    mocked_404 = SynapseHTTPError("Not Found", response=Mock(status_code=404))
    with patch.object(
        CREATE_CLS, "_create_challenge", side_effect=mocked_404
...
def test_get_or_create_challenge__get_raise_missing_param():
    ...
    teamid = str(uuid.uuid1())
    with pytest.raises(
        TypeError, match=".*missing 1 required positional argument: " "'projectId'"
    ):
...
def test__set_acl():
    ...
    entity = Mock()
    acl_config = [
        {"principal_id": "1111111", "access_type": ["READ", "DOWNLOAD"]},
        {"principal_id": "2222222", "access_type": ["READ", "DOWNLOAD", "UPDATE"]},
    ]
    expected_calls = [
        mock.call(
            entity=entity, principalId="1111111", accessType=["READ", "DOWNLOAD"]
        ),
        mock.call(
            entity=entity,
            principalId="2222222",
    ...
    with patch.object(syn, "setPermissions") as patch_set:
        create._set_acl(syn=syn, entity=entity, acl_config=acl_config)
