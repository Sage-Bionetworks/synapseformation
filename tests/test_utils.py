...
def test_synapse_login_default():
    ...
    ) as patch_synapse, patch.object(syn, "login") as patch_login:
...
def test_synapse_login_specify():
    ...
    with patch.object(synapseclient, "Synapse", return_value=syn) as patch_synapse:
        ...
        assert syn_conn == syn
        patch_synapse.assert_called_once_with(configPath=temp_config.name)
    temp_config.close()


@pytest.mark.parametrize(
    "error", [SynapseAuthenticationError, SynapseNoCredentialsError]
)
def test_synapse_login_error(error):
    """Test default synapse login config path"""
    syn = Mock()
    with patch.object(synapseclient, "Synapse", return_value=syn), patch.object(
        syn, "login", side_effect=error
    ), pytest.raises(ValueError, match=r"Login error: please make sure you .*"):
        utils.synapse_login()
